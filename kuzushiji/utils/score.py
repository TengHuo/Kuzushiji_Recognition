...
def detection_score(preds, truth):
    """
...
def detection_f1(sub, solution):
    ...
    results = pool.starmap(detection_score, zip(sub['labels'].values, solution['labels'].values))
    pool.close()
    pool.join()
...
__all__ = ['detection_f1', 'kuzushiji_f1']
